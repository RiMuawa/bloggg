...
public class SubscriptionDao {
    ...
    public List<Subscription> findAll(){
        return jdbc.query("SELECT * FROM subscriptions WHERE enabled=TRUE", this::mapRow);
    }
    ...
    public Long insert(Subscription s){
        jdbc.update("INSERT INTO subscriptions(bookmark_id,url,notify_email,period_hours,enabled,last_checked_at,last_content_hash) VALUES(?,?,?,?,?,?,?)",
    ...
    public void update(Subscription s){
        jdbc.update("UPDATE subscriptions SET bookmark_id=?,url=?,notify_email=?,period_hours=?,enabled=?,last_checked_at=?,last_content_hash=? WHERE id=?",
                s.getBookmarkId(), s.getUrl(), s.getNotifyEmail(), s.getPeriodHours(), s.isEnabled(), s.getLastCheckedAt(), s.getLastContentHash(), s.getId());
    }

    public void delete(Long id){ jdbc.update("DELETE FROM subscriptions WHERE id=?", id); }

    public Subscription findById(Long id){
        return jdbc.queryForObject("SELECT * FROM subscriptions WHERE id=?", this::mapRow, id);
    }
}
